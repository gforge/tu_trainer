...
class Hierarchical_Label(Base_Label[Modality_Csv_Single_Column_Cfg], Base_Output):
    ...
    #     target = unwrap(batch['results'][self.get_name()].pop('target')).reshape(-1)  # noqa: F841
    #     if ('pseudo_output' in batch['results'][self.get_name()]):
    #         pseudo_output = unwrap(batch['results'][self.get_name()].pop('pseudo_output'))  # noqa: F841
