...
from DataTypes import Modality_Csv_Single_Column_Cfg
...
class One_vs_Rest(Base_Label[Modality_Csv_Single_Column_Cfg], Base_Output):
    ...
    def set_runtime_values(self, value_dict, indices, num_views, prefix=''):
        for grp in value_dict.keys():
            self.set_runtime_value(runtime_value_name=f'{prefix}{grp}',
                                   value=value_dict[grp],
                                   indices=indices,
                                   num_views=unwrap(num_views))
    ...
    def analyze_modality_specific_results(self, batch):
        ...
        target = unwrap(batch['results'][self.get_name()].pop('target')).reshape(-1)
        ...
        values = {'accuracy': accuracy, 'prediction': prediction, 'entropy': entropy}
        self.set_runtime_values(value_dict=values, indices=batch['indices'], num_views=batch['num_views'])

        if ('pseudo_output' in batch['results'][self.get_name()]):
            pseudo_output = unwrap(batch['results'][self.get_name()].pop('pseudo_output'))
            ...
            self.set_runtime_values(value_dict={
                'accuracy': pseudo_accuracy,
                'prediction': pseudo_prediction,
                'entropy': self.compute_entropy(softmax=pseudo_softmax)
            },
    ...
    def report_modality_specific_epoch_summary(self, summary):
        ...
        if not self.has_runtime_value(name='pseudo_accuracy'):
            return
    ...
    def collect_statistics(self):
        ...
        tot = np.sum(self.label_stats['label_count'])
        if tot == 0:
            # All classes are missing - happens for id class during ImageNet training
            self.label_stats['label_likelihood'] = self.label_stats['label_count'] + 0.5
        else:
            self.label_stats['label_likelihood'] = self.label_stats['label_count'] / tot
    ...
    def get_num_classes(self):
        assert self.dictionary is not None, f'No dictionary has been initated for {self.get_name()}'
