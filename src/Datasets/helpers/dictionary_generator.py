...
def _build_empty_dictionary(values: List[str]) -> pd.DataFrame:
...
class Dictionary_Generator(metaclass=Singleton):
    """Manager for the unique values within the CSV:s
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.__dictionary: Dict[str, pd.DataFrame] = {}
        self.__values: Dict[str, List[str]] = defaultdict(list)
        self.__suggested_dictionaries: Dict[str, List[pd.DataFrame]] = defaultdict(list)

    def get(self, modality_name: str, action_on_missing='exception') -> Union[pd.DataFrame, None]:
        ...
        self.__dictionary[modality_name] = _build_empty_dictionary(values=self.__values[modality_name])
