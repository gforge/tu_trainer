...
class Fully_Connected(nn.Module):
    """
    Fully connected (FC) block creates a multilayer (or single layer)
    ...
    def __init__(
        ...
        hidden_volumes = np.linspace(np.log2(self.input_volume),
                                     np.log2(self.output_volume),
                                     num_hidden_layers + 2,
                                     dtype='float32')[1:-1]
    ...
    def forward(self, x):
        ...
        # The clone is required or we get a: "...is a view and is being modified inplace." error
        # This is not required when adding noise as that will automatically create a new variable
        x = x.view(batch_size, self.input_volume).clone()
        y = self.layers(x)
