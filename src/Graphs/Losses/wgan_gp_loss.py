...
from DataTypes import Loss_Wasserstein_GAN_GP_Loss_Cfgs

from global_cfgs import Global_Cfgs
from .base_divergent_loss import Base_Divergent_Loss


class Wasserstein_GAN_GP_Loss(Base_Divergent_Loss[Loss_Wasserstein_GAN_GP_Loss_Cfgs]):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.dummy_counter = 0
        self.coef = 1e-2  # Decrease loss to not overpower other losses

    @property
    def update_model_every_step(self) -> int:
        return self._cfgs.update_model_every_step
