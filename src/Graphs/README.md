...
3. `factory` is a factory that create singleton `encoder` and `decoder`. The reason that these neural networks are singelton is that we want to make sure that different graphs and tasks share the same mappings between modalities. (for example, during the train and test, we should have the same neural networks do the mappings)
