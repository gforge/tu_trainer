        ...
        for i in range(targets.shape[0]):
            for j in range(targets.shape[1]):
                if not np.any(np.isnan(targets[i])):
                    transformed_targets = SpatialTransform.from_pickled(
                        sp[i, j]).convert_original_points_to_processed_image_space(original_points=targets[i][j])
